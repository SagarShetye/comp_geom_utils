...
namespace MeshUtils {
	...
	// Apply a transform to the entire mesh
	void Halfedge_mesh::Transform(const Eigen::Matrix4f& transformMat) {
		if (transformMat == Eigen::Matrix4f::Identity())
			return;

		for (const auto& vertIdx : vertices()) {
			Point_3 vertPos = point(vertIdx);
			Eigen::Vector4f vertPosVec(vertPos.x(), vertPos.y(), vertPos.z(), 1.0);
			vertPosVec = transformMat * vertPosVec;
			vertPos = Point_3(vertPosVec(0), vertPosVec(1), vertPosVec(2));
		}
	}
}
